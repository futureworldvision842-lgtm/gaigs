...
(function(){
  ...
  const seedPosts=()=>state.settings?.showSampleData===true?[
    {id:'seed_creator_library',name:'Muhammad Qureshi',authorId:'seed-muhammad',scope:'Global',time:'Source-linked',type:'Creator archive',text:'Ideas, conversations, podcasts and field material for building transparent community systems. Review the original public collection before reusing any item.',location:'Pakistan',media:'Public creator library · original files on Google Drive',sourceUrl:'https://drive.google.com/drive/folders/1Jr_BeXJ-oZaSM3qPmAUvAOFy5GTpwZFX?usp=sharing',sourceLabel:'Open Muhammad’s public creator library'},
    {id:'seed_masjid',name:'Jamia Masjid Nabvi Community',authorId:'seed-masjid',scope:'Society',time:'Community source',type:'Community update',text:'A first working community space for construction updates, local participation and transparent public progress.',location:'Islamabad',media:'Community project · public website',sourceUrl:'https://masjidenabvimodel.netlify.app/',sourceLabel:'Open the Masjid community source'},
  ...
  function socialCard(post){
    ...
    const source=post.sourceUrl?`<a class="post-source-link" href="${esc(post.sourceUrl)}" target="_blank" rel="noopener noreferrer">↗ ${esc(post.sourceLabel||'Open original source')}</a>`:'';
    return `<article class="feed-post" data-post-id="${esc(id)}"><div class="post-head"><div class="avatar">${esc(post.initials)}</div><div><b>${esc(post.name)}</b><small>${esc(post.time||'Recently')} · ${esc(post.type||'Post')}${post.location?' · '+esc(post.location):''}</small></div><span class="scope-badge">${esc(post.scope||'City')}</span></div><div class="post-body">${esc(post.text||'')}${source}</div><div class="post-media">${attachment(post)}</div><div class="social-counts"><span>${reaction?'You reacted · ':''}${Object.keys(social.reactions).filter(k=>k===id).length||0} reactions</span><span>${comments} comments</span></div><div class="post-actions social-actions"><button class="${reaction?'active':''}" data-social-react="${esc(id)}">${reaction||'Support'}</button><button data-social-comments="${esc(id)}">Discuss</button><button class="${saved?'active':''}" data-social-save="${esc(id)}">${saved?'Saved':'Save'}</button>${mine?'':`<button data-social-follow="${esc(post.authorId)}">${following?'Following':'Follow'}</button><button data-social-menu="${esc(id)}">•••</button>`}</div></article>`;
  }
  views.feed=function(){
